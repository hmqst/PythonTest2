...
public class CameraActivity extends BaseActivity implements CameraDialog.CameraDialogParent {
    ...
    private ImageButton mCameraButton; // 开启/关闭USB相机预览
    private Surface mPreviewSurface;
    private ImageView mImageView; // 展示bitamp
    private Button takePic, back;
    private Bitmap bitmap; // 暂存bitamp
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 获取预览bitamp并传递给首页
        takePic = findViewById(R.id.camera_takePic);
        takePic.setOnClickListener(view -> {
            bitmap = mUVCCameraView.getBitmap();
            if (bitmap != null){
                mImageView.setImageBitmap(bitmap);
                MainActivity.bitmap = bitmap.copy(Bitmap.Config.ARGB_8888, true);
                Toast toast = Toast.makeText(CameraActivity.this,"保存成功，返回将自动处理该图片。",Toast.LENGTH_SHORT);
                synchronized (mSync) {
                    if (mToast != null) {
                        mToast.cancel();
                    }
        ...
        // 预览区域
        mUVCCameraView = findViewById(R.id.camera__view);
    ...
    private final USBMonitor.OnDeviceConnectListener mOnDeviceConnectListener = new USBMonitor.OnDeviceConnectListener() {
        ...
        public void onAttach(final UsbDevice device) {
            Toast.makeText(CameraActivity.this, "USB设备已连接", Toast.LENGTH_SHORT).show();
        }
...
//						camera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_RGB565/*UVCCamera.PIXEL_FORMAT_NV21*/);
        ...
        public void onDettach(final UsbDevice device) {
            Toast.makeText(CameraActivity.this, "USB设备已断开", Toast.LENGTH_SHORT).show();
        }
